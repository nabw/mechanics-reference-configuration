...
def prestress_solver_slab(mesh, markers, dirichlet_markers, neumann_markers, u_deg, n_times_back, n_times_forward, neumann_load, volumetric_load, xdmffile, solver_type=InverseFormulationType.direct, sellier_params=None, adaptive=False):
    ...
    bcs = [DirichletBC(V, Constant((0,0,0)), markers, b) for b in dirichlet_markers]
    ...
    if do_backward:
        if solver_type == InverseFormulationType.direct:
            FF = backwardForm(dx_mesh,ds_N, nn, u, v, ramp, neumann_load, volumetric_load)
            avg_nl, avg_l = inverseSolver(FF, times_backward, xdmffile, u, ramp, mesh, adaptive, bcs)
            ...
            FF = forwardForm(dx_mesh, ds_N, nn, u, v, ramp, neumann_load, volumetric_load)
            FF0 = backwardForm(dx_mesh,ds_N, nn, u, v, ramp, neumann_load, volumetric_load)
            avg_sell, avg_nl, avg_l = sellierSolver(FF, FF0, times_backward, xdmffile, u, ramp, mesh, sellier_params, adaptive, bcs)
    ...
    if do_forward:
        ...
        for t in times_forward:
            ...
            problem.solve()
        uforw.assign(u)
    saveSolution(mesh, coords, xdmffile, u, uinv, uforw, do_backward, do_forward)
...
def prestress_solver_cardiac(mesh, markers, endo_marker, epi_marker, base_marker, fibers, u_deg, n_times_back, n_times_forward, neumann_z, volumetric_z, pressure_known, active_stress_known, variational, xdmffile, solver_type=InverseFormulationType.direct, sellier_params=None, adaptive=False):
    ...
    if do_backward:
        if solver_type == InverseFormulationType.direct:
            ...
            avg_nl, avg_l = inverseSolver(FF, times_backward, xdmffile, u, ramp, mesh, adaptive)
        elif solver_type == InverseFormulationType.sellier:
            # One of the advantages of the Sellier algorithm is that it leverages the forward solver
            ...
            FF0 = backwardForm_cardiac(f0, s0, n0, dx_mesh, ds_base, ds_endo,
                              ds_epi, nn, u, v, ramp, pressure_known, active_stress_known, neumann_z, volumetric_z, variational=variational)
            avg_sell, avg_nl, avg_l = sellierSolver(FF, FF0, times_backward, xdmffile, u, ramp, mesh, sellier_params, adaptive)

        uinv.assign(u)
    
    if do_forward:
        u.vector().zero()  # Inverse is a bad initial point
        FF = forwardForm_cardiac(f0, s0, n0, dx_mesh, ds_base, ds_endo, ds_epi, nn, u, v, ramp, pressure_known, active_stress_known, neumann_z, volumetric_z)
        for t in times_forward:
            parprint("\t\t Forward ramp, t={:.4f}".format(t))
            ramp.t = t
            JJ = derivative(FF, u, TrialFunction(V)) 
...
def inverseSolver(FF, times_backward, xdmffile, u, ramp, mesh, adaptive, bcs=None):
    t = 0
    dt = 1 if len(times_backward) == 1 else times_backward[1] - times_backward[0]
        ...
        while error > ps_atol_inc and error / error0 > ps_rtol_inc and it < ps_max_it:
            if do_armijo and it > 0:
                ...
                for l in Ls:
                    ...
                    temp_incr.vector().axpy(-1, incr_prev.vector()) # This is R_l - R_{k-1}
            ...
            parprint("\tSellier inner iteration {}, err abs={:.3e}, err rel={:.3e}".format(it, error, error/error0))
            it += 1

        sell_its.append(float(it-1))
    return avg(sell_its), avg(nl_its), avg(l_its) # -1 as it goes up by one at the end
